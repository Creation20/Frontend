...
export default function DiagnosticScreen() {
  ...
  // Fixed: was named 'setSelectedIndex' but state holds an id string, not an index
  ...
  const selectedOption = TEST_OPTIONS.find((o) => o.id === selectedId);
        ...
        <SafeAreaView>
          <View style={styles.headerContent}>
            ...
            <Text style={styles.headerSubtitle}>
              Choose the style that is easiest for you to read.
            </Text>
          </View>
        </SafeAreaView>
      </LinearGradient>
        ...
        >
          <View
            style={[
              ...
              {
                backgroundColor: selectedOption?.previewColors[0],
                borderColor: '#E2E8F0',
                borderWidth: 1,
              },
            ]}
          >
            <Text
              style={[
                styles.previewText,
                {
                  color: selectedOption?.previewColors[1],
                  fontFamily:
                    selectedOption?.font === 'OpenDyslexic' ? 'OpenDyslexic' : 'System',
                  letterSpacing: selectedOption?.spacing,
                  lineHeight: 24 * ((selectedOption?.lineHeight ?? 1.8) / 1.5),
                },
              ]}
            >
              Reading is the gateway to learning. LexiAid uses artificial intelligence to
              make every sentence clear and every word accessible. Which of these styles
              feels most comfortable for your eyes?
            </Text>
          </View>
        </Animated.View>
            ...
            >
              <TouchableOpacity
                ...
                style={[
                  styles.optionBtn,
                  {
                    borderColor: selectedId === option.id ? '#0B6E6E' : '#E2E8F0',
                    borderWidth: selectedId === option.id ? 2 : 1,
                  },
                ]}
              >
                <View style={styles.optionInfo}>
                  <Text
                    style={[
                      styles.optionLabel,
                      { color: selectedId === option.id ? '#0B6E6E' : '#1A1D2E' },
                    ]}
                  >
                    {option.label}
                  </Text>
